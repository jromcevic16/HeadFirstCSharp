    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            Dictionary<int, RetiredPlayer> retiredYankees = new Dictionary<int, RetiredPlayer>()
            {
                ...
                {42, new RetiredPlayer("Jacki Robinson", 1993) },
                {44, new RetiredPlayer("Reggie Jackson", 1993) },
            };
            foreach (int jersyNumber in retiredYankees.Keys)
            {
                RetiredPlayer player = retiredYankees[jersyNumber];
                Console.WriteLine($"{player.Name} #{jersyNumber} retired in {player.YearRetired}");
